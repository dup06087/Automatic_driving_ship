...
class boat:
    ...
    def thread_start(self):
        while True:
            ...
            try:
                print("in try nucloe")
                if self.serial_nucleo is None or not self.serial_nucleo_thread.is_alive():
                    ...
                    print("Started or restarted nucleo thread")
            except:
                # 연결 문제가 발생하면 serial_nucleo 연결을 끊습니다.
                if self.serial_nucleo and self.serial_nucleo.serial_port.is_open:
                    self.serial_nucleo.serial_port.close()
                self.serial_nucleo = None
                print("nucleo disconnected")

            try:
                print("in try gnss")

                # serial_gnss 초기화 및 쓰레드 시작
                if self.serial_gnss is None or not self.serial_gnss_thread.is_alive():
                ...
                # 연결 문제가 발생하면 serial_gnss 연결을 끊습니다.
                if self.serial_gnss and self.serial_gnss.serial_port.is_open:
                    self.serial_gnss.serial_port.close()
                self.serial_gnss = None
                print("gnss disconnected")
