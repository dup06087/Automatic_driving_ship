...
class Window(QMainWindow, form_class):
    def __init__(self):
        ...
        self.timer100.timeout.connect(self.auto_driving_timer)
        ...
        try:
            ...
            try:
                if self.worker.connection:
                    self.edit_IP.setText("223.171.136.213")
    ...
    def auto_driving_timer(self): # timer while문 필요 x
        if self.is_auto_driving:
            ...
            try:
                ...
                # if len(self.sensor_data['dest_latitude']) != len(self.sensor_data['dest_longitude']):
                #     raise ValueError("Mismatched lengths of dest_latitude and dest_longitude")
            ...
            try:
                self.worker.message = {"mode_jetson": "AUTO", "dest_latitude": float(destination[0]),
                                       "dest_longitude": float(destination[1])}
                # print("self.worker.message : ", self.worker.message)
    ...
    def simulator(self):
        try:
            ...
            float(self.sensor_data['dest_latitude'])
            float(self.sensor_data['dest_longitude'])
        except:
            return
