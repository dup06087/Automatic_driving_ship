            ...
            try:
                ...
                if (self.current_value['latitude'] is not None and self.current_value['longitude'] is not None and
                    ...
                    if self.flag_avoidance:
                        destination_latitude = float(self.way_point[0])
                        destination_longitude = float(self.way_point[1])
                    else:
                ...
                '''여기서 장애물 피할 필요 없으면 그대로 내보내고 
                회피 방향 선택했으면 그 방향출력 더 크게해서 피하기
                '''
            ...
            while True:
                ...
                try:
                    ...
                    while True:
                        ...
                        if ready_to_write:
                            keys_to_extract = ['latitude', 'longitude', 'velocity', 'heading', 'pitch', 'dest_latitude', 'dest_longitude']
                            # 새로운 딕셔너리 생성
                        ...
                        if ready_to_read:
                            print("ready to read")
                            try:
                                self.way_point = client_socket.recv(1024).strip()
