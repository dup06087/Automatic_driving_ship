...
class Worker(QtCore.QThread):
    def __init__(self):
        ...
        # self.recv_obstacle_host, self.recv_obstacle_port = '223.171.136.213', 5005
        self.recv_obstacle_host, self.recv_obstacle_port = '127.0.0.1', 5005
    ...
    def run(self):
        recv_socket = None
        send_socket = None
        recv_obstacle_socket = None
        print("Receiving readying")
        data_buffer = b''
        obstacle_data_buffer = b''
        while True:
            try:
                if not recv_socket:
                ...
                if not recv_obstacle_socket:
                    recv_obstacle_socket = self.create_socket(self.recv_obstacle_host, self.recv_obstacle_port)
                ...
                if recv_obstacle_socket is not None:
                    try:
                        _, ready_to_read_obstacle, _ = select.select([recv_obstacle_socket], [recv_obstacle_socket], [], 1)

                        self.handle_received_obstacle_data(ready_to_read_obstacle, recv_obstacle_socket, obstacle_data_buffer)
    ...
    def handle_received_obstacle_data(self, ready_to_read, recv_socket, data_buffer):
        try:
            if ready_to_read:
                data = recv_socket.recv(1024)
                print("raw received obstacle data : ", data)
                if data:
                    data_buffer += data
                    if b'\n' in data_buffer:
                        ...
                        self.obstacle_data = received_dict
                        self.jetson_socket_status = True

                        print("obstacles : ", self.obstacle_data)
