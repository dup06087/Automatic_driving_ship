...
def GNSS_get_data():
    ...
    while True:
        ...
        # serial_data = _get_bytes_stream(ser).decode()
        client_data = client_socket.recv(1024).decode()
        # 쌓인 데이터 처리도 한 번에 가능하도록 > 원리 : 최신 값으로 그냥 덮어씀, 오래된거 버림
        # "," 기준으로 파싱
        # NMEA_parsed = serial_data.split(",")
        NMEA_parsed = client_data.split(",")
        ...
        if ( serial_data == 'q' or serial_data == 'Q'):
            ser.close()
            break
        else:
            pass
        ...
        time.sleep(1) # 시리얼 수신함수 recv가 serial 포트에 데이터 들어올 때까지 기다리므로 짧게해줘서 GNSS 데이터 들어오면 바로 데이터 최신화
