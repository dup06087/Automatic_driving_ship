...
ellipse = Ellipse((0, 0), width=2*3*lat_cm_std, height=2*3*lon_cm_std, edgecolor='r', facecolor='none', linestyle='--', label='3 Std Deviation')
ax.add_patch(ellipse)

# 데이터 포인트 그리기 (인덱스가 클수록 alpha 값이 크도록, 시간 순서대로 불투명하게)
for i, (lat_cm, lon_cm) in enumerate(zip(adjusted_latitudes_cm, adjusted_longitudes_cm)):
    alpha_value = (i + 1) / len(adjusted_latitudes_cm)  # 인덱스에 따라 alpha 값 증가
    ax.plot(lat_cm, lon_cm, 'o', color='blue', alpha=alpha_value)

# 범례 항목 정의 및 추가
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='Data Points (Later = More Opaque)'),
...
ax.set_title('Coordinate (alpha by time index)')
ax.grid(True)
